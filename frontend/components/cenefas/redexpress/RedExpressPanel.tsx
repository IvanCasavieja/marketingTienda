...
import {
  Download, FileSpreadsheet, FileType2, Plus, Trash2, X, LayoutTemplate,
  ChevronDown, Check, Pencil, Loader2,
} from "lucide-react";
import { toolsApi, cenefasV2Api } from "@/lib/api";
...
// sacó el campo "Margen lateral" — quedó redundante con poder reposicionar
// a mano en el preview.
...
export default function RedExpressPanel() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (selectedTemplateId !== null) {
        fd.append("template_v1_id", selectedTemplateId.toString());
      } else {
        fd.append("template_upload", customPptx!);
      }
      // format_id no se manda — el backend usa el master_format que detecta
      // de la plantilla, que es lo único que funciona igual de bien para
      // diseños de hoja completa (A4) y de celda única (pinchos/3xA4).
  ...
  const canSubmit = !!excel && hasTemplate && !submitting;

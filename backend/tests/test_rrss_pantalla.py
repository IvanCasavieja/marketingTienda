...
# ---------------------------------------------------------------- (22/09) plurales, el artículo de la fuente, las columnas de la planilla

_PLURALES = ("terminado", "pausada", "placasMuyPesadas")


def test_los_plurales_los_hace_el_sistema_de_traducciones():
    """"Listo: 1 placas revisadas", "Faltan 1 placas", "No entraron 1 placas":
    i18next sabe hacer plurales (`_one` / `_other` con `count`), y las tres
    claves lo usan en los tres idiomas."""
    for idioma in _IDIOMAS:
        rrss = _rrss(idioma)
        for clave in _PLURALES:
            assert clave not in rrss, (idioma, clave, "la clave sin sufijo no deja elegir el plural")
            assert f"{clave}_one" in rrss and f"{clave}_other" in rrss, (idioma, clave)
            assert "{{count}}" in rrss[f"{clave}_other"]
            assert "{{count}}" not in rrss[f"{clave}_one"], (idioma, clave, "el singular no lleva el número")
    src = _texto(_REVIEW) + _texto(_UPLOAD)
    for clave in _PLURALES:
        assert f't("rrss.{clave}", {{ count:' in src, clave


def test_la_fuente_se_contrae_adentro_de_las_frases():
    """"Hay más de una fila de El mailing": se interpolaba el TÍTULO de la
    columna adentro de una oración. Cada idioma contrae distinto ("del
    mailing", "no mailing"), así que las formas viven en los JSON."""
    for idioma in _IDIOMAS:
        rrss = _rrss(idioma)
        for forma in ("fuenteNombre", "fuenteDe", "fuenteEn"):
            assert set(rrss[forma]) == {"mailing", "planilla"}, (idioma, forma)
        for texto in (rrss["sinParejaNinguna"], rrss["sinParejaAmbiguo"]):
            assert "{{fuente}}" in texto
            for preposicion in ("de {{fuente}}", "en {{fuente}}", "of {{fuente}}", "in {{fuente}}", "em {{fuente}}"):
                assert preposicion not in texto, (idioma, texto)
    src = _texto(_REVIEW)
    assert 'nombreFuente(t, v, "de")' in src and 'nombreFuente(t, v, "en")' in src
    assert 't("rrss.todoCoincide", { fuente: fuenteEnFrase })' in src
...
def test_la_pantalla_dice_que_la_fecha_y_la_leyenda_quedaron_sin_revisar():
    """Con planilla sin VIGENCIA y la fecha vacía en la carga, la fecha no se
    compara (comparador.comparar_elementos) y la pantalla no lo decía."""
    src = _texto(_REVIEW)
    assert 't("rrss.campoFecha")' in src and "v.config?.fecha" in src
    assert 't("rrss.legalAlcohol")' in src and "v.config?.legal_alcohol" in src
    for idioma in _IDIOMAS:
        assert _rrss(idioma)["campoFecha"]

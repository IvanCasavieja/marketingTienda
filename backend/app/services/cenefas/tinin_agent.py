...
- tipoOferta es el anuncio de la mecánica ("2x", "2da al 50%"). En Redexpres va grande arriba del precio; en Rompe del Finde va en la cocarda roja al costado. En M x N va VACÍA: ese literal lo lleva promoOferta (ver más abajo). Es TEXTO aunque traiga números: se imprime tal cual, no se le separa el decimal. OJO con el combo: ahí va SOLO la cantidad ("2x"), no "2x$299" — el precio lo pone precioOferta al lado (ver LA REGLA DE tipoOferta más abajo).
...
- precioOferta ES SIEMPRE UN PRECIO, en todos los mundos, y SIEMPRE sale de la columna PRECIO (el real por unidad): nunca lleva un literal de mecánica adentro ni un total calculado del texto de OFERTA. Lo que hay que comunicar además del precio -- el total de un combo, el literal de un M x N -- va en promoOferta, que el diseño dibuja SUPERPUESTA encima del cuadro del precio: con mecánica trae ese valor y tapa al precio, sin mecánica queda vacía y se ve el precio. Decisión de Ivan del 2026-08-26, ampliada a Combo el 2026-09-04 (antes Combo escribía el total en precioOferta y dejaba promoOferta vacía); el porqué está escrito arriba de todo en variables.py.
...
DOS REGLAS DE FONDO, que son las que más me equivocaba antes:
  (a) precioOferta ES UN PRECIO, SIEMPRE, y SIEMPRE sale de la columna PRECIO (el real por unidad) -- nunca de un cálculo sobre OFERTA, nunca un literal ni un texto.
  (b) lo que TAPA el cuadro del precio -- el total de un combo, el literal de un M x N -- es promoOferta. promoOferta se creó para liberar a precioOferta de tener que comunicar una promo.
- Combo ("2x$299") -> tipoOferta "2x" (SOLO la cantidad, NO el literal entero), ofertaUno VACÍA (ya no se autocompleta -- solo toma valor si alguien la mapea a mano en la pantalla de mapeo), precioOferta el de la columna PRECIO (el real por unidad), promoOferta el TOTAL (299, el número que sigue a la "x" en OFERTA), mecanica "Comprando 2, $149,50 la unidad.", tipoOfertaComprando "Comprando 2", unidad "unidad". El unitario de la mecánica sale de DIVIDIR el total (299/2), no de la columna PRECIO -- son dos números que pueden no coincidir. Decisión de Ivan del 2026-09-04: antes el TOTAL iba a precioOferta y promoOferta quedaba vacía; ahora el total se separó a promoOferta, igual que en M x N.
- M x N ("6x4", "2x1") -> tipoOferta VACÍA, promoOferta el literal ("2x1", es lo que tapa el cuadro del precio), precioOferta el número de la columna PRECIO (49,50), ofertaUno VACÍA, mecanica "$49,50 la unidad." Acá no hay un precio de oferta que anunciar —por eso el literal ocupa el cuadro— pero precioOferta igual lleva el unitario, que es un número. tipoOferta quedó VACÍA el 2026-09-16 por decisión de Ivan: el literal ya lo dice promoOferta, que va grande encima del precio, y ponerlo además en la cocarda lo repetía dos veces en el mismo cartel. OJO que en Combo NO es así: ahí la cocarda SÍ lleva "2x" y promoOferta lleva el total en pesos, que son dos datos distintos.
